...
function Mesh() {
    ...
    this.draw = function (r, g, b, half=false) {
        // code
        // bind vao
        gl.bindVertexArray(vao);

        // draw
        gl.vertexAttrib3f(WebGLMacros.VDG_ATTRIBUTE_COLOR, r, g, b);
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, vbo_index);
        if (half) { gl.drawElements(gl.TRIANGLES, numElements / 2, gl.UNSIGNED_SHORT, 0); }
        else { gl.drawElements(gl.TRIANGLES, numElements, gl.UNSIGNED_SHORT, 0); }
